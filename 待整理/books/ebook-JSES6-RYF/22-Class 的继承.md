...
上面代码中==`super`关键字，它在这里表示父类的构造函数，用来新建父类的`this`对象==。
...
最后，==父类的静态方法，也会被子类继承==。当然，这里也是子类的静态方法，不是子类的实例方法。
...
上面代码中，==`p`是父类`A`实例的属性，`super.p`就引用不到它==。所以，不能用过super直接调用父类的实例属性（constructor内部的属性），只能获取父类原型对象的属性。
...
ES6 规定，在==子类普通方法中通过`super`调用父类的方法时==，==方法内部的`this`指向当前的子类实例==。
...
（2）子类`prototype`属性的`__proto__`属性，表示==方法的继承==，总是指向父类的`prototype`属性。
