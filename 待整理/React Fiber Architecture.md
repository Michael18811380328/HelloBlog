...
介绍：React Fiber 架构是 React 核心算法的重构。Fiber 设计目的：提高动画，层级，手势等的适用性。主要功能是增量渲染，可以把渲染的工作分成不同的 trunks，然后在浏览器不同帧中渲染。其他关键更新：在更新组件时，如何暂停终端重用原来的内容。对不同的更新划分优先级。
...
关于本文：Fiber 引入了很多新术语，如果直接读源码比较难懂，这个文档是原作者（Andrew Clark）的笔记整理，不是官方文档。Fiber 还是一个正在进行的项目，可能在项目完成前，还需要经过重要的重构工作。
...
准备知识：学习 Fiber 前需要熟悉，组件元素实例（Component，Element，Instance），Reconciliation 算法（协调算法，DIFF 算法），React 基本理论概念；React 设计原则（状态驱动，scheduling 计划，与 Fiber 很重要）
...
复习：学习新概念前，需要复习 reconciliation 和 Scheduling
...
- reconciliation
  ...
  The algorithm React uses to diff one tree with another to determine which parts need to be changed.（协调算法：React 使用 reconciliation 算法实现两个树的 diff ，判断哪一部分改变）

- update

  A change in the data used to render a React app. Usually the result of `setState`. Eventually results in a re-render.（状态改变，驱动组件更新，造成重新渲染）

React 中心就是把整个组件重新渲染（render），这样程序员不需要考虑如何把一个元素改成另一个元素，而是直接关注状态的变化（进行声明式推理）

The central idea of React's API is to think of updates as if they cause the entire app to re-render. This allows the developer to reason declaratively, rather than worry about how to efficiently transition the app from any particular state to another (A to B, B to C, C to A, and so on).

实际上，重新更新整个 APP 仅适应于少数的测试组件，在生产环境中，这会造成大量性能浪费。React 在更新组件中，可以做到更新整个应用程序，但是保持较高的性能。这部分优化就是使用 reconciliation 算法。
