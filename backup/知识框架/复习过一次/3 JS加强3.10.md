...
对于简单的 if—else 结构，使用三目运算符（？）更方便。通常情况，代码较多更好理解，代码越少执行越快。
...
**在 JavaScript 中可通过—arguments .Length 来检查参数的个数**，需要放在被检测函数内部（判断函数的形参个数和实参个数）。
...
**全局变量**：script 标签内的所有变量；任何位置不使用 var 定义的变量。全局作用域只有一个，不管在哪个 script 标签内都是通用的。
