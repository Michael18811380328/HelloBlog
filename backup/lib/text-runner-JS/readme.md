...
# 运行帮助
michael$ node_modules/.bin/text-run help
...
michael$ node_modules/.bin/text-run setup
Created configuration file text-run.yml with default values

# 测试文件
michael$ node_modules/.bin/text-run test.md
...
# 新建一个JS文件并放入官方文档中的内容。
michael$ touch hello-world.js

# 加入自定义的命令
michael$ node_modules/.bin/text-run add hello-world
...
michael$ node_modules/.bin/text-run version
TextRunner v3.6.0
michael$ node_modules/.bin/text-run cd test.md
file or directory does not exist: cd
michael$ node_modules/.bin/text-run
