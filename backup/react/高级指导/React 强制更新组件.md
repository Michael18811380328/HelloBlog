...
有时候需要对一个组件进行强制更新（不是由于 state 变化造成组件更新）。react 原则上不需要强制进行更新，原理就是根据 state 和 props 进行数据传递，根据 diff 算法进行渲染组件得到局部更新。

那么强制更新就需要从 state 和 props 进行分析；如果逻辑问题就从生命周期函数角度分析。
