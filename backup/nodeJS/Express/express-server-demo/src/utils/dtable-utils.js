...
class DtableUtils {
  ...
  static updateDtableData(dtable_uuid, tableData, callback) {
    dTableWebAPI
      ...
      .then((res) => {
        ...
        form.append("file", fs.createReadStream(tableData.path), {
          filename: fileName,
        });
      })
      .catch((error) => {
  ...
  static recordOpID(dtable_uuid, version) {
    let sql =
      "INSERT INTO `operation_checkpoint` (dtable_uuid, op_id) VALUES \
  ...
  static listPendingOperationsByDTable(uuid, callback) {
    let sql =
      'SELECT a.operation, a.op_id FROM operation_log AS a LEFT JOIN \
              operation_checkpoint AS b ON a.dtable_uuid=b.dtable_uuid WHERE \
              a.dtable_uuid="' +
      uuid +
      '" AND a.op_id>b.op_id ORDER BY a.op_id';
  ...
  static listOperationsByDTable(uuid, callback) {
    let sql =
      'SELECT a.operation, a.op_id FROM operation_log AS a WHERE a.dtable_uuid="' +
      uuid +
      '"';
    DBHelper(sql, (err, results) => {
      if (err) {
