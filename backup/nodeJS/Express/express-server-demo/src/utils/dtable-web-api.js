...
class DTableWebAPI {
  ...
  getDownloadTableURL(dtable_uuid) {
    const url =
      this.dtableWebServiceURL +
      "api/v2.1/dtable-internal/get-file-download-link/";
  ...
  getUpdateTableURL(dtable_uuid) {
    const url =
      this.dtableWebServiceURL +
      "api/v2.1/dtable-internal/get-file-update-link/";
    let params = { dtable_uuid: dtable_uuid };
    const token = genJWT(dtable_uuid);
    return this.req.get(url, {
      headers: { Authorization: "Token " + token },
  ...
  getTableLatestCommitId(dtable_uuid) {
    const url =
      this.dtableWebServiceURL +
      "api/v2.1/dtable-internal/get-latest-commit-id/";
    let params = { dtable_uuid: dtable_uuid };
    const token = genJWT(dtable_uuid);
    return this.req.get(url, {
      headers: { Authorization: "Token " + token },
  ...
  getTableRelatedUsers(dtable_uuid) {
    const url =
      this.dtableWebServiceURL + "api/v2.1/dtable-internal/get-related-users/";
