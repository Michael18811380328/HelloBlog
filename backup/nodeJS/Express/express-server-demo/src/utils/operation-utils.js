...
class OperationUtils {
  static checkOperation(dtable, operation) {
    ...
    switch (op_type) {
      case OPERATION_TYPE.INSERT_ROW: {
        ...
        ({ isValid, error_message } = this._checkOperationParams(
          needParams,
          params
        ));
        if (!isValid) {
  ...
  static encapsulateOperation(dtable, operation) {
    ...
    switch (op_type) {
      case OPERATION_TYPE.INSERT_ROW: {
        let { op_type, table_id, row_id, row_data, row_insert_position } =
          operation;
        ...
        if (!row_id) {
          row_id =
            table.rows.length > 0
              ? table.rows[table.rows.length - 1]._id
              : null;
        }
      ...
      case OPERATION_TYPE.DELETE_ROW: {
        ...
        let upper_row_id =
          upper_row_index >= 0 ? table.rows[upper_row_index] : null;
