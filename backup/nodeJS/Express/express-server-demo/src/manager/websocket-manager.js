...
class WebSocketManager {
  ...
  onConnected(socket) {
        ...
        this.dtableManager.getDtable(dtable_uuid, (err, dtable) => {
          ...
          if (dtable) {
            ...
            callback &&
              callback(
                Message.success(
                  SUCCESS_TYPE.JOINED_SUCCESSFULLY,
                  dtableValue.version
                )
              );
          } else {
    ...
    socket.on("update-dtable", (dtable_uuid, operation, callback) => {
      ...
      try {
        let canOpApply = this.dtableManager.canOpApply(
          dtable_uuid,
        ...
        if (!canOpApply) {
          ...
          logger.error(
            "Operation can not be execute :" + ERROR_MESSAGE[error_type]
          );
        ...
        let nextVersion = this.dtableManager.execSocketOperation(
          dtable_uuid,
          JSON.parse(operation),
          username,
          appName
        ...
        callback &&
          callback(Message.success(SUCCESS_TYPE.UPDATE_COMPLETED, nextVersion));
    ...
    socket.on("disconnect", () => {
      ...
      if (this.appConnectionSocketMap.has(socket.id)) {
        if (this.appConnectionDTableMap.has(dtable_uuid)) {
          this.appConnectionDTableMap
            .get(dtable_uuid)
            .delete(this.appConnectionSocketMap.get(socket.id).app_name);
  ...
  recordAppConnection(payload, socket) {
    ...
    if (this.appConnectionDTableMap.get(payload.dtable_uuid)) {
      this.appConnectionDTableMap
        .get(payload.dtable_uuid)
        .add(payload.app_name);
    } else {
      this.appConnectionDTableMap.set(
        payload.dtable_uuid,
        new Set([payload.app_name])
      );
    }
  }
