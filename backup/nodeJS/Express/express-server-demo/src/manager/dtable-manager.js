...
class DTableManager {
                ...
                DtableUtils.updateDtableData(
                  ...
                  (error, dtableName) => {
                    ...
                    if (dtableName) {
                      ...
                      this.snapshotManager.snapshotDTable(
                        dtableUuid,
                        dtableName
                      );
                    } else {
                      logger.error(
                        "This can't happen. DTable saved but no dtableName is empty."
                      );
                    }
  ...
  execHttpOperation(username, appName, dtable_uuid, dtable, operation) {
    // valid
    let { isValid, error_message } = OperationUtils.checkOperation(
      dtable,
      operation
    );
    ...
    // send event message
    this.eventManager.publishEvents(
      dtable_uuid,
      dtable,
      newOperation,
      username,
    ...
    webSocketManager.io
      .in(dtable_uuid)
      .emit("update-dtable", execOperation, nextVersion);
  ...
  execSocketOperation(dtable_uuid, operation, username, appName) {
    ...
    if (dtable && dtable.value && dtable.value.tables) {
      ...
      logger.error(
        "DTable data loading error, please check your code to solve the error."
      );
      ...
      .catch((error) => {
        if (error.response) {
          if (error.response.data && error.response.data.error_msg) {
            callback &&
              callback(new Error(error.response.data.error_msg), null);
          } else {
  ...
  convertRows(dtable, table, view, rows) {
    const formulaColumns = Views.getAllFormulaColumns(
      Views.getColumns(view, table)
    );
    let formulaResults = {};
  ...
  insertRowToTable(username, appName, dtableUuid, dtable, options, callback) {
    ...
    let { isValid, error_message } = this.execHttpOperation(
      username,
      appName,
      dtableUuid,
      dtable,
  ...
  insertTableWithRawColumnsAndRows(
    dtable,
    dtable_uuid,
    table_name,
    raw_columns,
    ...
    raw_columns.forEach((col, index) => {
      ...
      let new_column = new GridColumn({
        column_key: new_column_key,
        column_type: CellType.TEXT,
        column_name: col,
      });
