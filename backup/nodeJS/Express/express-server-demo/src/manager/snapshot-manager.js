...
class SnapshotManager {
  ...
  snapshotDTable(dtableUuid, dtableName) {
    this.getDTableLatestSnapshot(
      dtableUuid,
      (err, latestCtime, latestCommitId) => {
        if (err) {
          ...
          logger.debug(
            `DTable ${dtableUuid} snapshot interval is less than one day`
          );
          return;
        } else {
          dTableWebAPI
            .getTableLatestCommitId(dtableUuid)
            .then((res) => {
              const commitId = res.data.latest_commit_id;
              if (latestCommitId === commitId) {
                logger.debug(
                  `DTable ${dtableUuid} snapshot ${commitId} already exists`
                );
                return;
              } else {
  ...
  queryDTableSnapshot(dtableUuid, callback) {
    let sql =
      'SELECT commit_id, ctime FROM dtable_snapshot WHERE \
              dtable_uuid="' +
      dtableUuid +
  ...
  recordDTableSnapshot(dtableUuid, dtableName, commitId) {
    let sql =
      "INSERT INTO `dtable_snapshot` (dtable_uuid, dtable_name, commit_id, ctime) VALUES (?, ?, ?, ?)";
