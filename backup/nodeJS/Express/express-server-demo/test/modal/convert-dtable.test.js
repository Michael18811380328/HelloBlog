      ...
      if (column.data && column.data.options) {
        column.data.options.forEach((option) => {
          expect(option).toEqual(
            expect.not.objectContaining({ ID: undefined })
          );
          expect(option).toEqual(
...
test("convert dtable data to version 1", () => {
  ...
  // format_version
  expect(dtableData1).toEqual(
    expect.objectContaining({ format_version: expect.any(Number) })
  );

  // id_row_map
  expect(dtableData1).toEqual(
    expect.not.objectContaining({ forms: expect.any(Object) })
  );

  // storage_version
  expect(dtableData1).toEqual(
    expect.not.objectContaining({ storage_version: expect.any(Number) })
  );
  ...
  dtableData1.tables.forEach((table) => {
    expect(table).toEqual(
      expect.not.objectContaining({ Id2Row: expect.any(Object) })
    );
...
test("normalize dtable", () => {
  ...
  dtableData1.tables.forEach((table) => {
    // _tid
    expect(table).toEqual(
      expect.objectContaining({ _tid: expect.any(String) })
    );
    ...
    // id_row_map
    expect(table).toEqual(
      expect.objectContaining({ id_row_map: expect.any(Object) })
    );
    ...
    // rows
    expect(table).toEqual(
      expect.objectContaining({ rows: expect.any(Object) })
    );
    ...
    table.views.forEach((view) => {
      // _vid
      expect(view).toEqual(
        expect.objectContaining({ _vid: expect.any(String) })
      );
      ...
      // is_locked
      expect(view).toEqual(
        expect.objectContaining({ is_locked: expect.any(Boolean) })
      );
      ...
      // formula_rows
      expect(view).toEqual(
        expect.objectContaining({ formula_rows: expect.any(Object) })
      );
      ...
      // filter_conjunction
      expect(view).toEqual(
        expect.objectContaining({ filter_conjunction: expect.any(String) })
      );

      // hidden_columns
      expect(view).toEqual(
        expect.objectContaining({ hidden_columns: expect.any(Array) })
      );
      ...
      // sorts
      expect(view).toEqual(
        expect.objectContaining({ sorts: expect.any(Array) })
      );
      ...
      // groupbys
      expect(view).toEqual(
        expect.objectContaining({ groupbys: expect.any(Array) })
      );
...
function addVaildCondition(dtableData) {
  dtableData.tables.forEach((table) => {
    table.views.forEach((view) => {
      ...
      if (
        view.hidden_columns &&
        Array.isArray(view.hidden_columns) &&
        view.hidden_columns.length > 0
      ) {
      ...
      if (
        view.groupbys &&
        Array.isArray(view.groupbys) &&
        view.groupbys.length > 0
      ) {
