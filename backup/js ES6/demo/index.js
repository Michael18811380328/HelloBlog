...
var nameresult = names.filter(function (item, index, array) {
  //返回数组，filter函数获取满足条件的项，map获取每一项计算值的集合，不改变原数组，forEach函数等价于for语句，对每项处理
  return index > 2;
});
printf(nameresult);
...
ages = [];
for (var i = 0; i < 7; i++) {
  //js没有块级作用域，for  if块内定义的变量，在块外可以访问，函数内定义的局部变量外部不可以访问。可以使用let i定义临时作用域，定义的变量和对外部变量的修改均不保留，
...
Object.defineProperty(student1, "name1", {
  //可以用于定义新数据属性，也可以修改原有数据属性。也可以不使用defineProperty可以直接定义数据属性。也可以使用defineProperties同时定义多个数据或访问器属性
...
Object.defineProperty(student1, "age", {
  //访问器属性，不能直接定义，必须通过defineProperty定义，不包含数据值，设置时调用set函数，读取时调用get函数。访问器属性名称不要和数据属性名称相同
  get: function () {
    return this._age;
  },
...
for (var myproperty in student1) {
  //for in遍历对象属性
  console.log(myproperty, ":", student1[myproperty]);
}

function Student(name, age) {
  //自定义函数，构造函数，等同于java中的自定义类。所有的类型派生于Object
  var sex = "男"; //函数内部为私有属性
  ...
  this.setName = function (name) {
    //函数不关心传入或者定义的参数数量和类型，因此所有函数没有重载
    if (typeof name == "string")
      //基本数据类型，做类型检验，避免参数传递错误
      this.name = name; //没有指定返回值，实际返回的是undefined
  };
  this.getAge = function () {
    if (this.age.toFixed(2) << 2)
      //转化为false的值："",0,NaN,null,underfined。其他转化为true，类型首字母大写，变量首字母小写，<<按位移动，<<<无符号按位移动,toFixed(2)表示保留2位小数
      return -~this.age; //~按位取非，&按位取与，|按位取或 ^按位取异或，一元减号，表示取负
    return this.age; //保证所有路径都有返回值，虽然不加也不会出错，因为有默认返回值undefined
  };
  this.setAge = function (age) {
    //函数参数总是按值传递，无论基本类型还是引用类型，引用类型传递引用的值，不传递指向对象的值
...
if (student2 instanceof Student)
  //instanceof判断变量是否是某个类型或其派生类型实例的，student1是Student类，同时也是Object
...
function Monitor() {
  //自定义函数，相当于自定义一个类，类名Monitor。在文件中多称为构造函数，相当于c++和java中的自定义类。每个函数类，都有基类Object
  this.task = ["学习"];
}
...
String.prototype.startwith = function (text) {
  //设置字符串引用的原型，为String、Object、Array等添加方法
...
function callfunction(myfunction, myargument) {
  //向函数传输函数引用
...
function getproperty(propertyname) {
  ...
  return function (object1) {
    //内部函数，返回函数引用，一个函数可以访问另一个函数的变量，叫做闭包。函数的this和arguments变量只搜索到活动对象中（活动上下文），不会一直向外层搜索
    printf("内层函数");
...
); //打开窗口,参数地址、窗口名或框架名、窗口属性。返回窗口引用，进而可控制窗口。window就是一个窗口引用。不过有可能弹窗会被屏蔽
...
if (wroxwin == null)
  ...
  var timeoutid = setTimeout(function () {
    //setTimeout设置超时调用。js是单线程语言。但可以设置超时调用和间歇调用
...
if (hint2 == hintarr[2]) printf("同一个元素");
...
if (document.implementation.hasFeature("Traversal", "2.0")) {
  ...
  var filter = function (node) {
    ...
    return node.tagName.toLowerCase() == "label"
      ...
      : NodeFilter.FILTER_SKIP; //在TreeWalker中还有NodeFilter.FILTER_REJECT表示跳过该节点及该节点的子节点
  };
  var iterator = document.createNodeIterator(
    ...
    filter,
    false
  ); //创建NodeIterator迭代器。参数：查询根节点，查询节点，过滤器。NodeFilter.SHOW_ELEMENT为查询节点类型为元素节点，可以使用|包含多种查询节点类型，
  var node = iterator.nextNode(); //第一个节点，
  while (node != null) {
    //最后一个节点的后续节点为null，第一个节点的前序节点为null
    printf(node.id);
  ...
  iterator = document.createTreeWalker(
    ...
    filter,
    false
  ); //创建TreeWalker迭代器，迭代器包含nextNode、previousNode、parentNode、firstChild、lastChild、nextSibling、previousSibling等方法
}
//范围
...
input1.onclick = function (event) {
  //事件也是元素对象的方法属性，可以直接设置和调用input1.onclick(),删除input1.onclick=null;
...
input1.addEventListener(
  ...
  true
); //参数：事件类型，函数引用，false表示冒泡段执行，true表示捕获段执行。addEventListener函数也可以为元素添加事件,不覆盖已有事件。removeEventListener删除事件，删除了必须和添加时是相同的函数引用。(本句中的匿名函数无法删除)
...
text1.onkeyup = function (event) {
  ...
  if (target.value.length == target.maxLength) {
    //value字段的值，maxLength属性
    var form = target.form; //form目标所属表单
    if (form.elements[1] && !form.elements[1].readOnly) {
      //elements表单元素集合，readOnly字段属性
...
myfile.onchange = function (event) {
  ...
  if (/image/.test(files[0].type)) {
    //test判断是否匹配，"名称"：files[0].name，"类型"：files[0].type，"大小"：files[0].size
    type = "image";
  ...
  reader.onprogress = function (event) {
    //每50ms更新一次进度
    if (event.lengthComputable) var rate = event.loaded / event.total;
    alert("加载比例" + rate);
  };
  reader.onload = function () {
    switch (type) {
      case "image":
        hintdiv.insertAdjacentHTML(
          "beforeend",
...
source.onmessage = function (event) {
  //open在连接建立时触发，message在接收到新数据时触发，error在无法建立连接时触发
...
printf("===========离线应用与客户端存储===========");
if (navigator.onLine)
  //检测离线还是在线。也可以通过window事件online和offline设置离线或在线。chrome11-即之前的版本始终为true
  printf("当前处于在线状态");
