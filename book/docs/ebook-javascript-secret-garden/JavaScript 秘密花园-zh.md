...
并且，当使用 [`for in`](http://bonsaiden.github.io/JavaScript-Garden/zh/#object.forinloop) 循环遍历对象的属性时，原型链上的**所有**属性都将被访问。
...
闭包是 JavaScript 一个非常重要的特性，这意味着当前作用域**总是**能够访问外部作用域中的变量。 因为 [函数](http://bonsaiden.github.io/JavaScript-Garden/zh/#function.scopes) 是 JavaScript 中唯一拥有自身作用域的结构，因此闭包的创建依赖于函数。
...
为了正确的获得循环序号，最好使用 [匿名包装器](http://bonsaiden.github.io/JavaScript-Garden/zh/#function.scopes)（**译者注：**其实就是我们通常说的自执行匿名函数）。
...
JavaScript 中局部变量只可能通过两种方式声明，一个是作为[函数](http://bonsaiden.github.io/JavaScript-Garden/zh/#function)参数，另一个是通过 `var` 关键字声明。
...
强烈推荐使用**严格等于操作符**。如果类型需要转换，应该在比较之前[显式](http://bonsaiden.github.io/JavaScript-Garden/zh/#types.casting)的转换， 而不是使用语言本身复杂的强制转换规则。

## `typeof` 操作符

`typeof` 操作符（和 [`instanceof`](http://bonsaiden.github.io/JavaScript-Garden/zh/#types.instanceof) 一起）或许是 JavaScript 中最大的设计缺陷， 因为几乎不可能从它们那里得到想要的结果。
...
为了避免上面复杂的强制类型转换，**强烈**推荐使用[严格的等于操作符](http://bonsaiden.github.io/JavaScript-Garden/zh/#types.equality)。 虽然这可以避免大部分的问题，但 JavaScript 的弱类型系统仍然会导致一些其它问题。
...
[定时函数](http://bonsaiden.github.io/JavaScript-Garden/zh/#other.timeouts) `setTimeout` 和 `setInterval` 都可以接受字符串作为它们的第一个参数。 这个字符串**总是**在全局作用域中执行，因此 `eval` 在这种情况下没有被直接调用。
