...
回调函数，就是把任务的第二段单独写在一个函数里面，等到重新执行这个任务的时候，就直接调用这个函数。回调函数的英语名字`callback`，直译过来就是"重新调用"。
...
第一段执行完以后，任务所在的上下文环境就已经结束了。在这以后抛出的错误，原来的上下文环境已经无法捕捉，只能当作参数，传入回调函数中处理。
...
Promise 的最大问题是==代码冗余==，原来的任务被 Promise 包装了一下，不管什么操作，一眼看去都是一堆`then`，原来的语义变得很不清楚。
...
编译器的“传名调用”实现，往往是==将参数放到一个临时函数之中==，==再将这个临时函数传入函数体==。这个临时函数就叫做 Thunk 函数。
...
你可能会问， Thunk 函数有什么用？ES6 有了 Generator 函数，Thunk 函数现在可以用于 Generator 函数的自动流程管理。
