...
数据结构三个要点：信息之间的逻辑关系，信息在计算机中的物理存储方式，对信息的运算（增删改查排序）。

数据的逻辑结构：分为线性结构（线性表）和非线性结构（树和图）。

数据的物理结构：顺序存储结构（逻辑上相邻的节点在物理存储上也相邻）、链式存储结构（逻辑上相邻的节点在物理上不一定相邻，节点的逻辑关系由指针描述）、索引存储结构（建立索引表来标识节点的地址）、散列存储结构（根据节点的关键字直接计算出节点的存储地址）。
...
时间复杂度：把算法中包含简单操作的次数叫做算法的时间复杂度，衡量算法的性能。大O表示法将简单操作的次数取极限获取（常见的优劣记住）。时间复杂度通常和数据量级有关：如果数据量级是亿，那么最差的算法复杂度是 O(n)，如果数据量级是万，最差的算法复杂度是 O(n^2)，如果数据量级是百，那么算法复杂度可以选择 O(n^3) 。

空间复杂度：计算过程中需要消耗的内存（存储 int 和 object 消耗内存不同）。在 JS 中，优先考虑时间复杂度，日常情况不需要考虑空间复杂度。如果操作的 Array 和 Object 超过十万，为了避免浏览器内存不足，需要考虑空间复杂度。JS 中的 Array 和 Object 实际占用内存相似。C 中 array 比 object 占用内存少。
